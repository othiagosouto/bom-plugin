...
internal class ProjectExtKtTest {
    private lateinit var project: Project
    private val bomConfiguration = "bomConfiguration"
    private val implementation = "implementation"
    private val soutoPackage = "dev.thiagosouto"
    @Before
    fun setup() {
    ...
    fun `should parse dependencies with  as expected`() {
        ...
        expectedDependencies.add(Dependency("plugin", soutoPackage, "1.3.4"))
        expectedDependencies.add(Dependency("sample", soutoPackage, "1.3.5"))
    ...
    fun `should build dependency with exclusion rules`() {
        val expectedDependencies = mutableListOf<Dependency>()
        expectedDependencies.add(
            Dependency(
                "plugin",
                "dev.thiagosouto",
                "1.3.4",
                listOf(Exclusion("test1", soutoPackage))
            )
        )

        project.dependencies.add(bomConfiguration, "dev.thiagosouto:plugin:1.3.4") {
            this.exclude(mapOf("group" to soutoPackage, "module" to "test1"))
        }
